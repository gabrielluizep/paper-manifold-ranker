...
structure of relative municipal performance. By ignoring Pareto dominance
...
scalar ranking score through pairwise Pareto dominance-guided learning on a
...
The graph is constructed using \gls{knn} on $\Xmodel$ with $k=15$ and Euclidean distance
...
biases, $\deg_i$ is the in-degree of node $i$, and $\epsilon$ is a small stability
...
employed. At each hop, neighbor lists are randomly subsampled to a fixed limit of 5,
...
\begin{equation}
  \begin{aligned}
    \mathcal{L} &= \lambda_\mathrm{comp} \mathcal{L}_\mathrm{comp} + \lambda_\mathrm{recon} \mathcal{L}_\mathrm{recon} \\
    &\quad + \lambda_\mathrm{rank} \mathcal{L}_\mathrm{rank} + \lambda_\mathrm{geo} \mathcal{L}_\mathrm{geo},
  \end{aligned}
\end{equation}
...
\noindent
where the margin $m=0{.}1$ serves a dual purpose:
it demands a minimum score
