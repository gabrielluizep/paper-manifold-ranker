...
\begin{abstract}
    ...
    ranking guided by a Pareto-inspired proxy in the absence of labeled
    rankings. The architecture employs residual graph convolutional layers and
    is trained with a multi-objective loss function that enforces pairwise
    ranking consistency, input reconstruction fidelity, geometric smoothness on
    the learned manifold, and directional alignment with a Pareto-inspired
    proxy. The approach is validated on a longitudinal dataset covering
    Brazilian municipalities over multiple monthly periods, totaling over 82,000
    municipality--period observations. Results show that the proposed model
    produces scores with a wider effective range ($\sigma=0.098$) than ANATEL's
    legacy QoS ($\sigma=0.054$) and its revised version ($\sigma=0.058$)
    baselines, improving rank discrimination. Distributional and spatial
    analyses indicate that the framework produces more informative and
    distinguishable municipality rankings, mitigating the concentration effects
    observed in current regulatory composite indicators.
\end{abstract}
...
International best practices advocate results-based regulatory approaches,
including for service quality regulation. In this context, and given the
multidimensional nature of \gls{qos} indicators, municipality ranking is
essential to support the prioritization of regulatory actions. Recent works have
explored data-driven approaches that go beyond traditional aggregation,
including graph-neural-network-based \gls{qos}
...
characteristic of multi-objective optimization problems, where no single
solution simultaneously optimizes all
criteria~\cite{suresh2024machine,kesireddy2024elite}. Establishing a definitive
...
one~\cite{haxhiraj2025pareto}. However, many municipality pairs are incomparable
under strict Pareto ordering, motivating the search for a principled
...
near-identical values. As shown, the legacy IQS concentrates 50\% of
observations between 0.835 and 0.863, an interquartile range of only 0.028.
...
optimization landscape than unidimensional scalar approaches.
...
municipality--period entities and $D$ corresponds to the number of features.
$\Xraw$ is indexed by two keys: \textit{month}, which identifies the observation
period, and a unique \textit{municipality identifier}. Each row, therefore,
corresponds to one municipality--month pair. Since not all selected features are
available at monthly granularity, interpolation schemes are applied where
necessary.

The eight indicators available for the mobile service were selected as \gls{qos}
features and normalized to $[0,1]$: $\mathrm{IND}_1$ (voice-call connection
rate), $\mathrm{IND}_2$ (voice-call drop rate), $\mathrm{IND}_3$
(data-connection rate), $\mathrm{IND}_4$ (download and upload speed compliance
rate), $\mathrm{IND}_5$ (data-connection latency), $\mathrm{IND}_6$ (jitter),
$\mathrm{IND}_7$ (packet-loss rate), and $\mathrm{IND}_8$ (service availability
rate). As additional \gls{qos} information we provided the
$\mathrm{INF4}_{\mathrm{UP},\{\mathrm{3G,4G,5G}\}}$ and
$\mathrm{INF4}_{\mathrm{DL},\{\mathrm{3G,4G,5G}\}}$ indicators, which record the
median upload and download speeds over each mobile data technology. For
\gls{qoe}, the $\mathrm{ISG}$ (General Satisfaction Index) was selected as a
composite of \gls{qoe} indicators for a given municipality--period tuple.
...
scores\footnote{Referred to by \gls{anatel} as \gls{iqs} (Índice de Qualidade do
    Serviço).}. The \gls{iqs} is officially published on a semi-annual basis,
indexed by provider and municipality, with the primary objective of monitoring
and evaluating telecommunications service quality in Brazil through a
transparent and comparative indicator of provider
...
level by averaging across providers, both composites linearly combining
$\mathrm{IND}_1$ through $\mathrm{IND}_8$. The legacy composite $\iqsleg =
...
inverse-harmonic weight $\alpha_r = 1/(r\,H_K)$, where $H_K =
    ...
    \alpha_r\cdot\mathrm{IND}_{\pi(r)} + B_{\mathrm{cov}}$, with $\pi$ ordering
indicators from worst to best and $B_{\mathrm{cov}}$ a five-point bonus awarded
when 4G or higher coverage reaches at least 95\% in the urban area
\cite{anatel2025ri444}.

\section{Methodology} \label{sec:methodology}

The proposed ranking framework consists of a pipeline that transforms raw
municipality indicators into a scalar ranking score through pairwise
dominance-guided learning on a graph-structured representation.
Fig.~\ref{fig:pipeline} presents an overview of the process, which is detailed
as follows.
...
input to the model, and $\Xpareto$, used as a directional proxy during training.
The pipeline applies a Log1p transform to compress heavy tails, followed by
Robust Scaling (median and interquantile range $\{0.05, 0.95\}$) to reduce the
influence of outliers, and Standard Scaling to yield $\Xmodel$ with zero mean
and unit variance, ensuring that no single feature dominates gradient updates
owing to differences in magnitude or spread. In parallel, Min-Max scaling on
$\Xlog$ produces $\Xpareto \in [0,1]^{N \times D}$, from which a scalar
surrogate utility $u_i$ is derived as the feature-wise mean---a heuristic linear
scalarization that provides a consistent monotone direction for training in the
absence of labeled rankings.
...
constructed over the preprocessed feature space $\Xmodel$. Each
...
metric, yielding an index matrix $\mathcal{I} \in \{1,\ldots,N\}^{N \times k}$
where each row $\mathcal{I}_{i,:}$ is the neighbor list for node $i$. The
resulting graph is directed: edges are generated as $(i \rightarrow j)$ for each
$j \in \mathcal{I}_{i,:}$.
...
desirable, it also means the network lacks a notion of global coordinates on the
dataset manifold. To inject this structural awareness, we compute positional
encodings $p_i \in \mathbb{R}^{d_{pe}}$.

We employ \gls{pca}, which finds the orthogonal directions of maximum variance
in $\Xmodel$ and projects onto the top components, generating a linear
approximation of the manifold's global coordinates.

While spectral methods based on the graph Laplacian eigenvectors provide richer
positional encodings by capturing both local and global graph structure,
\gls{pca} is adopted here as a computationally efficient linear approximation
suitable for the dataset scale.

The computed features $p_i$ are standardized and concatenated with the original
node data $x_i$, ensuring that the positional coordinates do not dominate owing
to disproportionate magnitudes.
...
feature matrix $X \in \mathbb{R}^{n \times d_{in}}$ for a subgraph with $n$
nodes and edge list in COO format, each ResGCN layer computes
...
\noindent
where the skip path is either the identity (when dimensions match) or a linear
projection, this residual structure shortens gradient paths that improve
trainability and mitigate oversmoothing---the convergence of node
...
processing device at each step.

All activations use the Gaussian Error Linear Unit (GELU), achieving smooth,
continuously differentiable non-linearity. GELU is monotone increasing for
positive and small negative values, coupled with the model components, satisfies
monotonicity constraints: positive variations in individual \gls{qos} metrics
consistently translate to non-decreasing scores.
...
complementary perspectives, yielding a more balanced and robust solution than
...
constraints via margin-based pairwise comparisons, following the classical
RankNet formulation~\cite{burges2005learning} and its recent neural
...
utility values of municipalities $A$ and $B$, respectively
...
edges, following the discrete Dirichlet energy on the graph
...
Since graph edges connect municipalities that are similar in feature space, this
term softly enforces that nearby entities receive comparable scores. A
conservative weight on this term prevents oversmoothing, preserving meaningful
score variation across the distribution.
...
municipality--period observations. The legacy composite $\iqsleg$ concentrates
scores in a narrow band ($\sigma = 0.054$, \gls{iqr} $= 0.028$), and the revised
$\iqsrev$ offers only marginal improvement ($\sigma = 0.058$, \gls{iqr} $=
    0.028$). In contrast, the proposed score $\hat{r}$ achieves $\sigma = 0.098$ and
\gls{iqr} $= 0.137$---a $1.8\times$ and nearly $5\times$ expansion relative to
$\iqsleg$, respectively---enabling finer rank differentiation between
municipalities with distinct performance profiles.

Fig.~\ref{fig:hist} presents overlaid histograms of the three scoring
approaches, enabling direct comparison of their frequency distributions across
the score domain. The $\iqsleg$ distribution is comparatively more clustered:
...
wider but displays a pronounced asymmetry, with a tall bar near $0.59$ and a
...
To assess whether the proposed score reflects meaningful socioeconomic structure
beyond the telecommunications indicators used as input, we adopt the \gls{mhdi}
as an external validation variable---independent of the intra-set features. The
proposed score achieves a Pearson correlation of $\rho = 0.40$ with the
\gls{mhdi}, an increase of $0.07$ and $0.05$ over the correlation obtained by
the baselines with the same index. Although moderate in absolute terms, this
gain indicates that the proposed model captures socioeconomic structure not
encoded in the linear composites, lending external credibility to the ranking
beyond what intra-set agreement alone could establish.
...
To spatially validate the framework, December 2022 was selected as the
evaluation frame corresponding to the period with the fewest missing values
across municipalities. Fig.~\ref{fig:mps} shows the comparative geographic
mapping for each metric across Brazilian municipalities. While the proposed
model score exhibits continuous performance transitions that highlight distinct
geographic regions---consistent with the distributional patterns observed in the
histogram analysis---the regulatory indicators collapse the vast majority of
...
flatten the distribution---masks known infrastructure and socioeconomic
...
performance-weighted scheme, still exhibits limited score dispersion, with most
of the territory concentrated in a narrow range---even though broad regional
contrasts, such as the separation between the North and the remaining regions,
remain discernible. The proposed framework amplifies this variation, extending
discriminability across the full distribution of the municipality.

In contrast, the proposed model's score map reveals a clear North--South
gradient consistent with the known human development landscape across Brazilian
...
scores aligned with their documented development challenges~\cite{undp2023mhdi}.
Importantly, even within macro-regions, the model captures intra-regional
heterogeneity: isolated high-performing municipalities (typically state capitals
or large urban centers) are clearly visible against their surrounding rural
areas, a distinction that the baselines' compressed ranges cannot render.
...
limits spatial and technical discrimination, this paper introduces a
self-supervised, Pareto-guided ranking framework that significantly expands
score discriminability across the municipality distribution, surpassing legacy
and revised baselines in both spatial and technical separation through joint
optimization of competing quality objectives.

By combining pairwise ranking, feature reconstruction, geometric smoothness, and
a directional compass penalty, the model's optimization covers distinct,
complementary dimensions, yielding a more balanced solution than that provided
by one-dimensional scalar optimization. The quantitative analysis confirms the
proposed score achieves an interquartile range of $0.137$, representing a nearly
five times expansion compared to both baselines---\gls{iqr} $= 0.028$ for
...
further corroborates these findings: the proposed score correlates with the
index at $\rho = 0.40$, exceeding the baselines' correlation by $0.07$ and
$0.05$, indicating that the learned ranking captures socioeconomic structure
beyond what the linear composites encode.
...
extra-set covariates independent of telecommunications is also a planned
direction. Furthermore, the approach can also be extended to other socioeconomic
domains. Finally, exploring alternative positional encoding methods, such as
Spectral Embedding for non-linear manifold coordinates, and testing other
activation functions to assess infrastructural stability, and remain planned
efforts for subsequent research.
