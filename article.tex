...
Section~\ref{sec:conclusions} makes some final remarks.
...
\begin{equation}
    ...
    \label{eq:qos_leg}
\end{equation}
where $y_{\max} = 10$ is the upper bound of the satisfaction scale, each
$\mathrm{IND}_k \in [0,1]$ is a normalized technical indicator, and the weight vector $w^{\mathrm{leg}} \in \Delta^{K-1}$
...
Fig.~\ref{fig:hst} illustrates the probability distributions for the three metrics. The histogram clearly demonstrates that the baseline composites are heavily clustered in narrow regions: $\iqsleg$ presents a sharp peak near $0.85$, and $\iqsrev$ concentrates around $0.58$--$0.60$. This significant score aggregation reflects a loss of discrimination when evaluating municipalities with similar, generally satisfactory performance. In contrast, the model's score $s$ is broadly spread over the domain, exhibiting a distribution whose largest concentration lies near $0.55$--$0.65$ with visible spread across the full range. This structural dispersion explicitly favors rank discrimination, indicating that the multi-objective optimization successfully captures diverse technical variations between observations.

\begin{figure}[t]
    \centering
    \includegraphics[width=1\linewidth]{figures/hist.pdf}
    \caption{Score distribution of $\iqsleg$, $\iqsrev$, and the proposed score $s$. The model achieves a wider spread, enhancing discriminability.}
    \label{fig:hst}
\end{figure}

To spatially validate the framework, we consider a single evaluation frame representing the period of December 2022. Fig.~\ref{fig:mps} shows the comparative geographic mapping for each metric across Brazilian municipalities. The map visualization corroborates the distributional patterns: while the \gls{gnn} score exhibits continuous performance transitions that clearly highlight distinct geographic regions, the traditional indicators cluster the vast majority of municipalities into the highest or a narrow middle tier. This saturation in the baselines makes it difficult to distinguish between well-performing and under-performing regions, whereas the proposed model reveals fine-grained spatial heterogeneity.
